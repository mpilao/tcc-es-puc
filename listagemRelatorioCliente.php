...
if(!empty($_GET['order'])) {
    switch ($_GET['order']) {
        case 'payment':
            $order_by = " ORDER BY dataProximoPagamento ASC";
            break;
        case 'name':
            $order_by = " ORDER BY nome ASC";
            break;
...
$view               = file_get_contents( "views/listagemRelatorioCliente.tmpl" );
$view_row_client    = file_get_contents( "views/inc/listagemClienteRelatorio_linha.tmpl" );
$view_header_client = file_get_contents( "views/inc/listagemClienteRelatorio_header.tmpl" );

$menu               = include "menu.php";
$view               = str_replace("<@MENU>", $menu, $view);

$rs                 = $db->execute("SELECT * FROM Cliente, Pagamento WHERE Cliente.idCliente = Pagamento.idCliente" . $order_by);
$results            = $rs->getRows();
